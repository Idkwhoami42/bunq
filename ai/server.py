...
from models.task import ChatResponse, Pot, Activities
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=get_prompt(
                request.participants, request.pot, State.PRE_POT_CREATION
            )
        ),
    )
...
def event_started(request: ChatRequest):
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=get_prompt(
                request.participants, request.pot, State.EVENT_STARTED
            ),
    ...
    # return the locations for each activity
